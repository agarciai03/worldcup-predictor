...
const STAGE_WEIGHT = {
  ...
  "Round of 16": 0.85, "Quarter-final": 0.92, "Semi-final": 0.97, "Final": 1.0,
}

const RIVALRIES = new Set([
  "ARG-BRA", "BRA-ARG", "ENG-GER", "GER-ENG", "ESP-POR", "POR-ESP",
  "FRA-ENG", "ENG-FRA", "NED-GER", "GER-NED", "ITA-FRA", "FRA-ITA",
  "MEX-USA", "USA-MEX", "ARG-ENG", "ENG-ARG",
])

function getTeam(id) { return TEAMS.find(t => t.id === id) }

function predictMatch(home, away) {
  const homeStrength = (home.attack * 0.35 + home.midfield * 0.3 + home.defense * 0.35) * home.form + (50 - home.fifa_rank) * 0.3 + 3
  const awayStrength = (away.attack * 0.35 + away.midfield * 0.3 + away.defense * 0.35) * away.form + (50 - away.fifa_rank) * 0.3
  const diff = homeStrength - awayStrength
  const sigmoid = x => 1 / (1 + Math.exp(-x))
  const pHome = sigmoid(diff / 8) * 0.75 + 0.1
  const pAway = sigmoid(-diff / 8) * 0.75 + 0.05
  const pDraw = Math.max(0.05, 1 - pHome - pAway)
  const total = pHome + pDraw + pAway
  const hg = Math.max(0.3, (home.attack - away.defense) / 18 + 1.2)
  const ag = Math.max(0.3, (away.attack - home.defense) / 18 + 1.0)
  return {
    home_win: +(pHome / total).toFixed(4),
    draw: +(pDraw / total).toFixed(4),
    away_win: +(pAway / total).toFixed(4),
    expected_score: { home: +hg.toFixed(2), away: +ag.toFixed(2) },
    predicted_score: { home: Math.round(hg), away: Math.round(ag) },
  }
}

function watchabilityScore(prediction, home, away, match) {
  const uncertainty = 1 - Math.max(prediction.home_win, prediction.draw, prediction.away_win)
  const competitive = uncertainty * 100
  const hg = prediction.expected_score.home
  const ag = prediction.expected_score.away
  const goalExcitement = Math.min(100, (hg + ag) * 22)
  const starQuality = (home.attack + away.attack) / 2
  const stageFactor = (STAGE_WEIGHT[match.stage] || 0.7) * 100
  const rivalry = RIVALRIES.has(`${home.id}-${away.id}`) ? 100 : 60
  const raw = competitive * 0.28 + goalExcitement * 0.22 + starQuality * 0.18 + stageFactor * 0.22 + rivalry * 0.10
  const score = Math.round(Math.min(100, raw) * 10) / 10
  const tier = score >= 82 ? "MUST WATCH" : score >= 70 ? "RECOMMENDED" : score >= 58 ? "OPTIONAL" : "BACKGROUND"
  return {
    score, tier,
    breakdown: {
      competitiveness: Math.round(competitive * 10) / 10,
      goal_potential: Math.round(goalExcitement * 10) / 10,
      star_power: Math.round(starQuality * 10) / 10,
      stage_weight: Math.round(stageFactor * 10) / 10,
      rivalry,
    }
  }
}

const MATCHES = MATCH_PAIRS.map(([home, away, stage, venue], i) => {
  const base = new Date(2026, 5, 30, 18, 0)
  const kickoff = new Date(base.getTime() + (Math.floor(i / 2) * 86400000) + ((i % 2) * 3 * 3600000))
  return {
    id: `m${i + 1}`,
    home, away, stage, venue,
    kickoff: kickoff.toISOString(),
    status: i > 1 ? "upcoming" : i === 0 ? "live" : "soon",
  }
})
...
function enrichMatch(m) {
  ...
  const watchability = watchabilityScore(prediction, home, away, m)
  return { ...m, home_team: home, away_team: away, prediction, watchability }
}

export function mockResponse(path, options) {
  if (path === '/teams') return TEAMS
  if (path.startsWith('/teams/')) {
    const id = path.split('/')[2]
    const team = getTeam(id)
    return { ...team, players: PLAYERS.filter(p => p.team === id) }
  }
  if (path.startsWith('/players?')) {
    const sortBy = new URLSearchParams(path.split('?')[1]).get('sort_by') || 'rating'
    return [...PLAYERS].sort((a, b) => b[sortBy] - a[sortBy])
  }
  if (path.startsWith('/players/')) {
    const id = path.split('/')[2]
    const p = PLAYERS.find(x => x.id === id)
    return { ...p, team_info: getTeam(p.team) }
  }
  if (path === '/matches') return MATCHES.map(enrichMatch)
  if (path.startsWith('/matches/')) {
    ...
    return {
      ...enriched,
      home_squad: PLAYERS.filter(p => p.team === m.home),
      away_squad: PLAYERS.filter(p => p.team === m.away),
    }
  }
  if (path.startsWith('/recommendations')) {
    const limit = parseInt(new URLSearchParams(path.split('?')[1]).get('limit') || '5')
    return MATCHES.map(enrichMatch).sort((a, b) => b.watchability.score - a.watchability.score).slice(0, limit)
  }
  if (path === '/predict') {
    const body = JSON.parse(options.body)
    const home = getTeam(body.home)
    const away = getTeam(body.away)
    const fakeMatch = { id: 'custom', home: body.home, away: body.away, stage: body.stage || 'Group A', venue: 'Simulator', kickoff: '', status: 'simulation' }
    const prediction = predictMatch(home, away)
    return { home_team: home, away_team: away, prediction, watchability: watchabilityScore(prediction, home, away, fakeMatch) }
  }
  if (path === '/stats/summary') {
    return {
      teams_count: TEAMS.length,
      players_tracked: PLAYERS.length,
      matches_scheduled: MATCHES.length,
      total_goals: PLAYERS.reduce((s, p) => s + p.goals, 0),
      total_assists: PLAYERS.reduce((s, p) => s + p.assists, 0),
      top_scorer: [...PLAYERS].sort((a, b) => b.goals - a.goals)[0],
      top_assister: [...PLAYERS].sort((a, b) => b.assists - a.assists)[0],
      top_rated: [...PLAYERS].sort((a, b) => b.rating - a.rating)[0],
    }
  }
  return null
}
